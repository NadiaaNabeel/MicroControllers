...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
  if (htim->Instance == TIM4) // Using your TIM4 setup
  {
    ...
    // // Integral
    // integral += error * dt;
    // if (fabsf(error) < 0.5f) integral = 0.0f;
    ...
    // Set_Motor_Speeds(-pid_output);
     if (tilt_angle > 40.0f || tilt_angle < -40.0f) 
    {
        // WE FELL OVER. KILL THE MOTORS.
    ...
    {
        // WE ARE UPRIGHT. RUN THE PID MATH.
        float error = setpoint - tilt_angle;
        ...
        filtered_derivative = alpha * raw_derivative + (1.0f - alpha) * filtered_derivative;
        float D = Kd * filtered_derivative;
        pid_output = P + I + D;
        previous_error = error;

        // Note: If the robot darts in the WRONG direction and forces itself to crash, 
        // change this to Set_Motor_Speeds(pid_output);
        Set_Motor_Speeds(-pid_output); 
    }
    counter++;
